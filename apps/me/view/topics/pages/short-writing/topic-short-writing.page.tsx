	...
	const shortWritingPosts: { [key: string]: { name: string; posts: TPost[]; description?: string } } = {
		journaling: {
			...
			posts: sortPostsByDate(
				shortWriting?.filter((post) => post.published && post.type === 'journaling')?.slice(0, 4),
				'desc'
			),
		...
		buddhism: {
			name: 'Buddhism',
			posts: sortPostsByDate(
				shortWriting?.filter((post) => post.published && post.type === 'buddhism')?.slice(0, 4),
				'desc'
			),
		...
		'i-ching': {
			name: 'I Ching',
			posts: sortPostsByDate(
				shortWriting?.filter((post) => post.published && post.type === 'i-ching')?.slice(0, 4),
				'desc'
			),
		...
		'ba-zi': {
			...
			posts: sortPostsByDate(
				shortWriting?.filter((post) => post.published && post.type === 'ba-zi')?.slice(0, 4),
				'desc'
			),
