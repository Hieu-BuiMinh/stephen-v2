...
function ConnectionsBento({ linkTo }: IConnectionsBento) {
	...
	return (
		<BentoCard className="group overflow-hidden size-full min-h-[210px]" linkTo={linkTo}>
			...
			>
				<BackgroundPattern />
				<div className="absolute inset-y-0 left-0 z-20 w-1/3 bg-gradient-to-r from-background to-transparent" />
				<div className="absolute inset-y-0 right-0 z-20 w-1/3 bg-gradient-to-l from-background to-transparent" />

				<Image
					className="absolute left-1/2 top-[48.5%] size-[100px] -translate-x-1/2 -translate-y-1/2 transform rounded-full"
...
export default ConnectionsBento
...
function ConnectionCircle({
	...
	return (
		<div className={cn('border-bg-secondary rounded-full border bg-muted size-16 p-1', className)}>{children}</div>
	)
}
