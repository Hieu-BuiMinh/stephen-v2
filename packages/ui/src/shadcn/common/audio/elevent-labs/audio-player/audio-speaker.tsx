 ...
 * Copyright (c) elevenlabs.io: https://ui.elevenlabs.io/blocks
 * Source:
 *  - https://ui.elevenlabs.io/blocks
 *  - https://v0.app/chat/open-in-v0-ENkTLjoKpy2
 *
 * Modified by: Stephen
 */
	...
	return (
		<AudioPlayerButton
			...
			item={
				player.activeItem
					? {
							id: exampleTracks?.[currentTrackIndex] ? exampleTracks?.[currentTrackIndex].id : '0',
							src: exampleTracks?.[currentTrackIndex]?.url ? exampleTracks?.[currentTrackIndex].url : '',
							data: {
								name: exampleTracks?.[currentTrackIndex]?.name
									? exampleTracks?.[currentTrackIndex].name
									: '',
							},
						}
					: undefined
...
const SpeakerContextBridge = ({ className }: { className?: string }) => {
	...
	return useMemo(() => <SpeakerControls className={className} playerRef={playerRefStatic} />, [className])
}
...
const SpeakerOrbsSection = memo(
	({ isDark, audioDataRef }: { isDark: boolean; audioDataRef: React.RefObject<number[]> }) => {
		return (
...
function SpeakerControls({
	...
	useEffect(() => {
		const track = {
			id: exampleTracks[0] ? exampleTracks[0].id : '0',
			src: exampleTracks[0] ? exampleTracks[0].url : '',
			data: {
				name: exampleTracks[0] ? exampleTracks[0].name : 'Unknown Track',
			},
		}
		playerApiRef.current.setActiveItem(track)
		precomputeWaveform(track.src)
	}, [playerApiRef, precomputeWaveform])
	...
	}, [ambienceMode, playerApiRef, setupAudioContext])
	...
	const playTrack = useCallback(
		(index: number) => {
			...
			const track = {
				id: exampleTracks[index] ? exampleTracks[index].id : '',
				src: exampleTracks[index] ? exampleTracks[index].url : '',
				data: { name: exampleTracks[index] ? exampleTracks[index].name : '' },
			}
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							<div className="min-w-0 flex-1">
								<h3 className="text-foreground truncate text-sm font-medium">{currentTrack?.title}</h3>
								<Link
						...
						<div
							...
							onTouchStart={(e) => {
								...
								const handleMove = (e: TouchEvent) => {
									...
									if (timeDelta > 0) {
										const instantVelocity = (touch ? touch.clientX : 0 - lastClientX) / timeDelta
