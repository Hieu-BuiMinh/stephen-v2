 ...
 * Copyright (c) elevenlabs.io: https://ui.elevenlabs.io/docs/components
 * Source:
	...
	useEffect(() => {
		...
		const animate = (currentTime: number) => {
			...
			for (let i = 0; i < barsRef.current.length; i++) {
				...
				;(barsRef.current[i] as any).x -= speed * deltaTime
			}
			...
			while (
				...
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				(barsRef.current[barsRef.current.length - 1] as any).x < rect.width
			) {
			...
			try {
				...
				const updateData = () => {
					...
					for (let i = halfLength - 1; i >= 0; i--) {
						const value = Math.min(1, (relevantData[i] || 0 / 255) * sensitivity)
